...
// El de servicio vive aparte: el scraper corre fuera de Next y no puede
// arrastrar next/headers.
const clienteAdmin = async () => (await import('./supabase-admin.js')).supabaseAdmin();
 ...
 */
export async function sincronizar(fuente, todasCrudas) {
  const soloCampos = (p) => Object.fromEntries(CAMPOS.map((c) => [c, p[c] ?? null]));

  // Una fuente puede repetir el mismo id en una corrida (CheapShark devuelve
  // el mismo juego en varias tiendas). Postgres rechaza el lote entero con
  // "ON CONFLICT ... cannot affect row a second time", así que se deduplica acá
  // y no en cada scraper.
  const promos = [...new Map(todasCrudas.map((p) => [p.id, p])).values()];
