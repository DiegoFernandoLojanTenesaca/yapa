import * as produbanco from '../scrapers/produbanco.js';
import { supabaseAdmin } from './supabase.js';

export const FUENTES = [produbanco];

const UA =
  'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';

const CAMPOS = [
  'id', 'fuente', 'banco', 'comercio', 'titulo', 'detalle',
  'categoria', 'ciudad', 'vence', 'codigo', 'url', 'imagen',
];

const soloCampos = (p) => Object.fromEntries(CAMPOS.map((c) => [c, p[c] ?? null]));

/**
 * Vuelca lo scrapeado respetando el trabajo del admin:
 *  - una promo marcada `editada` no se pisa, solo se reactiva
 *  - `publicada` y `destacada` se conservan al actualizar
 *  - lo que desapareció del origen queda `activa = false`, no se borra
 */
async function sincronizar(sb, fuente, promos) {
  const { data: previas, error } = await sb
    .from('promos')
    .select('id, editada, publicada, destacada')
    .eq('fuente', fuente);
  if (error) throw new Error(`leyendo previas: ${error.message}`);

  const antes = new Map((previas ?? []).map((p) => [p.id, p]));
  const vistos = new Set(promos.map((p) => p.id));

  const aEscribir = [];
  const aReactivar = [];

  for (const p of promos) {
    const previa = antes.get(p.id);
    if (previa?.editada) {
      aReactivar.push(p.id);
      continue;
    }
    aEscribir.push({
      ...soloCampos(p),
      activa: true,
      // Conserva las decisiones del panel; en las nuevas, valores por defecto.
      publicada: previa ? previa.publicada : true,
      destacada: previa ? previa.destacada : false,
      actualizada: new Date().toISOString(),
    });
  }

  if (aEscribir.length) {
    const { error } = await sb.from('promos').upsert(aEscribir, { onConflict: 'id' });
    if (error) throw new Error(`guardando: ${error.message}`);
  }
  if (aReactivar.length) {
    await sb.from('promos').update({ activa: true }).in('id', aReactivar);
  }

  const desaparecidas = [...antes.keys()].filter((id) => !vistos.has(id));
  if (desaparecidas.length) {
    await sb.from('promos').update({ activa: false }).in('id', desaparecidas);
  }

  return {
    encontradas: promos.length,
    nuevas: promos.filter((p) => !antes.has(p.id)).length,
    archivadas: desaparecidas.length,
  };
}

async function correrUna(sb, f) {
  const res = await fetch(f.url, { headers: { 'user-agent': UA }, cache: 'no-store' });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);

  const promos = f.scrape(await res.text());

  // Si el banco rediseña su web el parser devuelve 0, y sincronizar() dejaría
  // todo el catálogo de esa fuente archivado. Mejor fallar ruidosamente.
  if (promos.length === 0) throw new Error('0 promos: cambió el HTML, revisar selectores');

  return sincronizar(sb, f.fuente, promos);
}

/** Corre todas las fuentes. Una que falle no tumba a las demás. */
export async function correrTodo() {
  const sb = supabaseAdmin();
  const reporte = [];

  for (const f of FUENTES) {
    try {
      const r = await correrUna(sb, f);
      await sb.from('corridas').insert({ fuente: f.fuente, ...r, error: null });
      reporte.push({ fuente: f.fuente, ok: true, ...r });
    } catch (e) {
      await sb.from('corridas').insert({ fuente: f.fuente, error: e.message });
      reporte.push({ fuente: f.fuente, ok: false, error: e.message });
    }
  }
  return reporte;
}
